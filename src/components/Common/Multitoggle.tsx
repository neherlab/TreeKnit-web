...
export const SwitchLabel = styled.label<{ $widthPx: number; $checked: boolean }>`
  position: relative;
  z-index: 2;
  float: left;
  width: ${(props) => props.$widthPx}px;
  line-height: 26px;
  font-size: 12px;
  color: ${({ $checked, theme }) => ($checked ? theme.white : theme.bodyColor)};
      ...
      {values.map((val) => {
        return (
          <span key={val}>
            ...
            <ClickableLabel value={val} $checked={val === value} onChange={onChange} itemWidth={itemWidth} />
          </span>
        )
      })}
